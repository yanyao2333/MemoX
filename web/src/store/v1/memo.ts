...
import {
	...
	type GetMemosFromPastMonthsRequest,
...
export const useMemoStore = create(
	combine(getDefaultState(), (set, get) => ({
		...
		getPastMonthsMemos: async (request: GetMemosFromPastMonthsRequest) => {
			const { memos } = await memoServiceClient.getMemosFromPastMonths(request);
			const memoMap = { ...get().memoMapByName };
			for (const memo of memos) {
				memoMap[memo.name] = memo;
			}
