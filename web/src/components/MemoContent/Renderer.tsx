...
const Renderer: React.FC<Props> = ({ index, node }: Props) => {
  switch (node.type) {
    ...
    case NodeType.PARAGRAPH:
      return (
        <Paragraph index={index} {...(node.paragraphNode as ParagraphNode)} />
      );
    case NodeType.CODE_BLOCK:
      return (
        <CodeBlock index={index} {...(node.codeBlockNode as CodeBlockNode)} />
      );
    ...
    case NodeType.HORIZONTAL_RULE:
      return (
        <HorizontalRule
          index={index}
          {...(node.horizontalRuleNode as HorizontalRuleNode)}
        />
      );
    case NodeType.BLOCKQUOTE:
      return (
        <Blockquote
          index={index}
          {...(node.blockquoteNode as BlockquoteNode)}
        />
      );
    case NodeType.LIST:
    ...
    case NodeType.ORDERED_LIST_ITEM:
      return (
        <OrderedListItem
          index={index}
          {...(node.orderedListItemNode as OrderedListItemNode)}
        />
      );
    case NodeType.UNORDERED_LIST_ITEM:
      return (
        <UnorderedListItem
          {...(node.unorderedListItemNode as UnorderedListItemNode)}
        />
      );
    case NodeType.TASK_LIST_ITEM:
      return (
        <TaskListItem
          index={index}
          node={node}
          {...(node.taskListItemNode as TaskListItemNode)}
        />
      );
    ...
    case NodeType.EMBEDDED_CONTENT:
      return (
        <EmbeddedContent
          {...(node.embeddedContentNode as EmbeddedContentNode)}
        />
      );
    case NodeType.TEXT:
    ...
    case NodeType.STRIKETHROUGH:
      return (
        <Strikethrough {...(node.strikethroughNode as StrikethroughNode)} />
      );
    case NodeType.MATH:
    ...
    case NodeType.ESCAPING_CHARACTER:
      return (
        <EscapingCharacter
          {...(node.escapingCharacterNode as EscapingCharacterNode)}
        />
      );
    ...
    case NodeType.REFERENCED_CONTENT:
      return (
        <ReferencedContent
          {...(node.referencedContentNode as ReferencedContentNode)}
        />
      );
