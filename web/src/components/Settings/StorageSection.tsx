...
const StorageSection = () => {
  ...
  const [workspaceStorageSetting, setWorkspaceStorageSetting] =
    useState<WorkspaceStorageSetting>(
  ...
  const allowSaveStorageSetting = useMemo(() => {
    ...
    if (
      ...
      WorkspaceStorageSetting_StorageType.LOCAL
    ) {
      ...
      workspaceStorageSetting.storageType ===
        WorkspaceStorageSetting_StorageType.S3 &&
  ...
  const handleMaxUploadSizeChanged = async (
  ...
  const handleFilepathTemplateChanged = async (
    event: React.FocusEvent<HTMLInputElement>
  ) => {
    const update: WorkspaceStorageSetting = {
      ...workspaceStorageSetting,
  ...
  const handlePartialS3ConfigChanged = async (
    s3Config: Partial<WorkspaceStorageSetting_S3Config>
  ) => {
    const update: WorkspaceStorageSetting = {
      ...workspaceStorageSetting,
    ...
    <div className="flex w-full flex-col gap-2 pt-2 pb-4">
      ...
      <RadioGroup
        ...
        onChange={(event) => {
          handleStorageTypeChanged(
            event.target.value as WorkspaceStorageSetting_StorageType
          );
        }}
      >
        <Radio
          value={WorkspaceStorageSetting_StorageType.DATABASE}
          label={t('setting.storage-section.type-database')}
        />
        <Radio
          value={WorkspaceStorageSetting_StorageType.LOCAL}
          label={t('setting.storage-section.type-local')}
        />
      ...
      <div className="flex w-full flex-row items-center justify-between">
        <div className="flex flex-row items-center">
          ...
          <Tooltip
            title={t('setting.system-section.max-upload-size-hint')}
            placement="top"
          >
            <HelpCircleIcon className="h-auto w-4" />
          </Tooltip>
        </div>
        <Input
      ...
      {workspaceStorageSetting.storageType !==
        WorkspaceStorageSetting_StorageType.DATABASE && (
        <div className="flex w-full flex-row items-center justify-between">
          <span className="mr-1 text-gray-700 dark:text-gray-500">
            Filepath template
          </span>
      ...
      <div>
        <Button
          color="primary"
          disabled={!allowSaveStorageSetting}
          onClick={saveWorkspaceStorageSetting}
        >
          {t('common.save')}
        </Button>
      </div>
        ...
        <List component="ul" marker="disc" size="sm">
          ...
          <ListItem>
            <Link
              ...
              target="_blank"
            >
              Choosing a Storage for Your Resource: Database, S3 or Local
              Storage?
            </Link>
          </ListItem>
        </List>
