...
const CreateIdentityProviderDialog: React.FC<Props> = (props: Props) => {
  ...
  const [type, setType] = useState<IdentityProvider_Type>(
  ...
  useEffect(() => {
    if (identityProvider) {
      ...
      if (identityProvider.type === IdentityProvider_Type.OAUTH2) {
        const oauth2Config = OAuth2Config.fromPartial(
          identityProvider.config?.oauth2Config || {}
        );
  ...
  const allowConfirmAction = () => {
    ...
    if (type === 'OAUTH2') {
      ...
      if (isCreating && oauth2Config.clientSecret === '') {
        return false;
      }
    }
  ...
  const handleConfirmBtnClick = async () => {
    try {
      if (isCreating) {
        ...
        toast.success(
          t('setting.sso-section.sso-created', { name: basicInfo.title })
        );
      } else {
        ...
        toast.success(
          t('setting.sso-section.sso-updated', { name: basicInfo.title })
        );
      }
    } catch (error: any) {
    ...
    <>
      ...
      <div className="dialog-content-container min-w-[19rem]">
        {isCreating && (
          <>
            ...
            <Select
              ...
              onChange={(_, e) => setSelectedTemplate(e ?? selectedTemplate)}
            >
        ...
        {type === 'OAUTH2' && (
          <>
            {isCreating && (
              <p className="mb-2 w-full break-all rounded-md border p-2 text-sm">
        ...
        <div className="mt-2 flex w-full flex-row items-center justify-end space-x-1">
